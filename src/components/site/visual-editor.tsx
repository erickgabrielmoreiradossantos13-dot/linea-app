...
import { ImportedSiteCanvas, type ImportedMediaTarget, type ImportedTextChange } from "@/components/site/imported-site-canvas";
import { addBlock, duplicateBlock, moveBlock, publishWebsite, removeBlock, restoreOriginal, restoreVersion, saveBlockDraftConfig, saveContentDraft, saveImportedBlockDraft, savePageSeoDraft, scheduleSitePublish, selectBlockImage, selectImportedImage, uploadEditorImage, uploadImportedImage, type EditorActionResult } from "@/app/(app)/site/[id]/edit/actions";

type Change =
  | { kind: "entry"; entryId: string; before: string; after: string }
  | { kind: "config"; blockId: string; before: BlockConfig; after: BlockConfig }
  | { kind: "imported"; blockId: string; itemId?: string; fieldId?: string; before: string; after: string };
...
export function VisualEditor({ data }: { data: EditorData }) {
  ...
  const [mediaTarget, setMediaTarget] = useState<ImportedMediaTarget | null>(null);
  ...
  const persistChange = (change: Change, target: "before" | "after") => {
    ...
    startTransition(async () => {
      const result = change.kind === "entry"
        ? await saveContentDraft({ entryId: change.entryId, value: value as string })
        : change.kind === "config"
          ? await saveBlockDraftConfig({ blockId: change.blockId, config: cleanConfig(value as BlockConfig) })
          : await saveImportedBlockDraft({ blockId: change.blockId, itemId: change.itemId, fieldId: change.fieldId, value: value as string });
      setStatus(result.error ? "error" : "saved"); if (result.error) setMessage(result.error); else if (change.kind === "imported") router.refresh();
    });
  };
  ...
  const undo = () => {
    ...
    else if (change.kind === "config") setBlocks((current) => current.map((block) => block.id === change.blockId ? { ...block, config: change.before } : block));
  ...
  const redo = () => {
    ...
    else if (change.kind === "config") setBlocks((current) => current.map((block) => block.id === change.blockId ? { ...block, config: change.after } : block));
  ...
  return <div className="visual-editor">
    ...
    {mediaTarget && <div className="editor-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setMediaTarget(null); }}><section className="editor-media-modal" role="dialog" aria-modal="true" aria-labelledby="media-title"><div className="editor-panel-title"><div><small>Imagem no design</small><h2 id="media-title">Substituir imagem</h2></div><button onClick={() => setMediaTarget(null)} aria-label="Fechar"><X size={18}/></button></div><form className="editor-upload-form" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); form.set("blockId", mediaTarget.blockId); if (mediaTarget.itemId) form.set("itemId", mediaTarget.itemId); if (mediaTarget.fieldId) form.set("fieldId", mediaTarget.fieldId); run(() => data.currentPage?.editorMode === "IMPORTED" ? uploadImportedImage(form) : uploadEditorImage(form)); setMediaTarget(null); }}><label>Arquivo<input name="file" type="file" accept="image/jpeg,image/png,image/webp,image/avif" required/></label><label>Texto alternativo<input name="altText" maxLength={500} placeholder="Descreve o que a imagem acrescenta"/></label><button className="editor-publish" disabled={pending}><ImageIcon size={15}/> Otimizar e usar</button></form><div className="editor-media-divider"><span>ou escolhe da biblioteca</span></div><div className="editor-media-grid">{data.media.map((item) => <button key={item.id} disabled={!item.url || pending} onClick={() => { run(() => data.currentPage?.editorMode === "IMPORTED" ? selectImportedImage({ blockId: mediaTarget.blockId, mediaId: item.id, itemId: mediaTarget.itemId, fieldId: mediaTarget.fieldId }) : selectBlockImage({ blockId: mediaTarget.blockId, mediaId: item.id })); setMediaTarget(null); }}><span>{item.url ? <img src={item.url} alt={item.altText ?? ""}/> : <ImageIcon/>}</span><strong>{item.filename}</strong><small>{item.altText || "Sem texto alternativo"}</small></button>)}{data.media.length === 0 && <p className="editor-empty-copy">Ainda não há imagens na biblioteca.</p>}</div></section></div>}
  </div>;
}
